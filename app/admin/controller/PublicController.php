        ...
        if (!empty($result) && $result['user_type'] == 1) {
            if (cmf_compare_password($pass, $result['user_pass'])) {
                $groups = Db::name('RoleUser')
                    ...
                    ->join('__AREA__ c', 'b.area_id =c.id','LEFT')
                    ->where(["user_id" => $result["id"], "status" => 1])
                    ->field("role_id,b.area_id,c.name as area_name,c.path,c.dwxz")->find();
                if ($result["id"] != 1 && (count($groups)==0 || empty($result['user_status']))) {
                    $this->error("用户被禁用,请联系管理员!");
                }
                //登入成功页面跳转
                session('ADMIN_AREA_ID', $groups["area_id"]);
                session('ADMIN_AREA_NAME', $groups["area_name"]);
                session('ADMIN_AREA_PATH', $groups["path"]);
                session('ADMIN_AREA_DWXZ', $groups["dwxz"]);
