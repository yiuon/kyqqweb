...
class NavMenuModel extends Model
{
     ...
     * @return array
     */
    public function navMenusTreeArray($navId = 0, $maxLevel = 0, $dwxz = 0, $area_id = 0, $pagename = "")
    {
        if (empty($navId)) {
        ...
        $where["b.dwxz"] = ["eq",$dwxz];
        
        $navMenus     = $this->alias('a')->join("navmenu_dwxz b","a.id = b.navmenu_id","LEFT")->field("distinct a.*")->where($where)->where('a.nav_id', $navId)->where('a.status', 1)->order('a.list_order ASC')->select()->toArray();
        $navMenusTree = [];
        if (!empty($navMenus)) {
            $tree = new Tree();
            $this->parseNavMenu4Home($navMenus,$area_id,$pagename,$dwxz);
            $tree->init($navMenus);
    ...
    private function parseNavMenu4Home(&$navMenus,&$area_id,&$pagename,&$dwxz)
    {
        foreach ($navMenus as $key => $navMenu) {
            ...
            if (strpos($hrefOld, "{") !== false) {
                ...
                if ($area_id!=0) {
                    $href['param']["areaid"] = $area_id;
                }
                if ($pagename!="") {
                    $href['param']["pagename"] = $pagename;
                }
                if ($dwxz!=0) {
                    $href['param']["dwxz"] = $dwxz;
                }
