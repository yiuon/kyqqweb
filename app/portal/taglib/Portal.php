...
class Portal extends TagLib
{
    ...
    protected $tags = [
        ...
        'articles'         => ['attr' => 'field,where,limit,order,page,relation,returnVarName,pageVarName,categoryIds,dwxz,areaid', 'close' => 1],//非必须属性item
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $areaid = empty($tag['areaid']) ? 0 : $tag['areaid'];
        $dwxz = empty($tag['dwxz']) ? 0 : $tag['dwxz'];
        $field = "''";
        if (!empty($tag['field'])) {
        ...
        $areaid = "''";
        if (!empty($tag['areaid'])) {
            if (strpos($tag['areaid'], '$') === 0) {
                $areaid = $tag['areaid'];
                $this->autoBuildVar($areaid);
            } else {
                $areaid = "'{$tag['areaid']}'";
            }
        }
        ...
        $dwxz = "''";
        if (!empty($tag['dwxz'])) {
            if (strpos($tag['dwxz'], '$') === 0) {
                $dwxz = $tag['dwxz'];
                $this->autoBuildVar($dwxz);
            } else {
                $dwxz = "'{$tag['dwxz']}'";
            }
        }
