...
class Tree
{
     ...
     * @return string
     */
    public function getTree($myId, $str, $sid = 0, $adds = '', $str_group = '',$now_id=0)
    {
        $number=1;
        //一级栏目
        $child = $this->getChild($myId,$number);
        if (is_array($child)) {
            $total = count($child);
            ...
            foreach ($child as $key => $value) {
                if($now_id==0){
                    ...
                    $this->getTree($id, $str, $sid, $adds . $k . $nbsp, $str_group,0);
                    $number++;
                }else if($value['id'] == $now_id){
                    $j = $k = '';
                    if ($number == $total) {
                        $j .= $this->icon[2];
                    } else {
